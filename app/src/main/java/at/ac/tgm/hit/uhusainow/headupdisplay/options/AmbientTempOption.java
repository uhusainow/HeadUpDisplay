...
public class AmbientTempOption extends Option {

    public static final int AMBIENTTEMP_UPDATE_TIME = 120000;

    public AmbientTempOption(Activity activity, BluetoothSocket bluetoothSocket){
    ...
    public void createContent(int zoneId, int zoneContentId, int option) {
        ...
        }, AmbientTempOption.AMBIENTTEMP_UPDATE_TIME, AmbientTempOption.AMBIENTTEMP_UPDATE_TIME);
    ...
    public void updateContent(int zoneContentId) {
        ...
        AmbientAirTemperatureCommand ambientTmp = new AmbientAirTemperatureCommand();
        ambientTmp.useImperialUnits(false);

        try {
            ambientTmp.run(super.getBluetoothSocket().getInputStream(), super.getBluetoothSocket().getOutputStream());
        } catch (IOException e) {
            e.printStackTrace();
        ...
        final int tmpZoneContentId = zoneContentId;
        final String ambientTmpString = ambientTmp.getFormattedResult();
        super.getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {

                TextView content = (TextView) getActivity().findViewById(tmpZoneContentId);
                content.setText(ambientTmpString);
