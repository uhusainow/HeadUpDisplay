...
public class FuelOption extends Option {

    public static final int FUEL_UPDATE_TIME = 120000;

    public FuelOption(Activity activity, BluetoothSocket bluetoothSocket){
    ...
    public void createContent(int zoneId, int zoneContentId, int option) {
        ...
        }, FuelOption.FUEL_UPDATE_TIME, FuelOption.FUEL_UPDATE_TIME);
    ...
    public void updateContent(int zoneContentId) {
        ...
        try {
            fuelLevel.run(super.getBluetoothSocket().getInputStream(), super.getBluetoothSocket().getOutputStream());
        } catch (IOException e) {
            e.printStackTrace();
        ...
        final int tmpZoneContentId = zoneContentId;
        final String percentage = fuelLevel.getFormattedResult();
        super.getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {

                TextView content = (TextView) getActivity().findViewById(tmpZoneContentId);
                content.setText(percentage);
