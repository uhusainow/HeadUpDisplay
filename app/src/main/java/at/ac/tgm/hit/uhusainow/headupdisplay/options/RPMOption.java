...
public class RPMOption extends Option {

    public static final int RPM_UPDATE_TIME = 500;

    public RPMOption(Activity activity, BluetoothSocket bluetoothSocket){
    ...
    public void createContent(int zoneId, int zoneContentId, int option) {
        ...
        }, RPMOption.RPM_UPDATE_TIME, RPMOption.RPM_UPDATE_TIME);
    ...
    public void updateContent(int zoneContentId) {
        ...
        try {
            rpm.run(super.getBluetoothSocket().getInputStream(), super.getBluetoothSocket().getOutputStream());
        } catch (IOException e) {
            e.printStackTrace();
        ...
        final int tmpZoneContentId = zoneContentId;
        final String rpmString = rpm.getFormattedResult();
        super.getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {

                TextView content = (TextView) getActivity().findViewById(tmpZoneContentId);
                content.setText(rpmString);
